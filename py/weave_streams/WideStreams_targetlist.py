...
def formatFits(
    ...
    kkk = np.setdiff1d(
        np.arange(len(source_id)), np.unique(source_id, return_index=True)[1]
    )
    source_id[kkk]
    ...
    print(
        "all, unique =",
        len(source_id),
        len(np.unique(source_id)),
        "shoud be equal now for WIDE",
    )
    print(
        "all, unique =",
        len(source_id[source_id != 0]),
        len(np.unique(source_id[source_id != 0])),
        "shoud be equal now for POINTED",
    )

    # %%
    # This is needed if using Gaia crossmatch; WSDB sets it to zero
    obj_id[obj_id == 999999] = 0
...
def parseExternalCatalogues(feature="sag", cfg=None, pointed=False):
    ...
    c = confLoad(f"{datadir}/external/{feature}.yaml")
    ...
    dfext = vaex.from_arrays(
        ra=ra, dec=dec, G=Gmag, source_id=source_id.astype(np.int64), priority=priority
    )
    ...
    if c["gaiarev"] > 2:
        edr3joined = edr3.join(
            dfext, left_on="source_id", right_on="source_id", rsuffix="_ext"
        )
    ...
    # If GDR2 do xmatch to update source_ids
        ...
        edr3joined = edr3.join(
            dfext, left_on="dr2_source_id", right_on="source_id", rsuffix="_ext"
        )
        jj = ~edr3joined.source_id_ext.isna()
        dfext_fixid = edr3joined[jj]
    ...
    # This eDR3 surce_id; external catalogue has _ext suffix
    source_id = dfext_fixid.source_id.values
