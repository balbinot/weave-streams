...
# Local imports
from weave_streams.utils.ebv import coef, get_SFD_dust
from weave_streams.utils.util import (_rename_wsdbcols, confLoad, fileExists,
                                      get_data, get_field_wsdb, mkpol, pc2mu)
...
def makecat(sname, output="default", config="config.yaml", pointedsurvey=False):
    ...
    if "sphi2" in t.keys():
        ...
        interp1d(t["phi1"], np.zeros_like(t["phi1"]) + c["dphi2"], bounds_error=False)
    ...
    ifeh, iage, label, iG, iBP, iRP, phase = np.loadtxt(
        f"{cfg['isocdir']}/Gaia/MIST_-1.5to-2.1.cmd",
        usecols=(7, 1, 0, 22, 23, 24, -1),
        unpack=True,
    )
    iifeh, iiage, llabel, ig, ir, ii, iz, pphase = np.loadtxt(
        f"{cfg['isocdir']}/PS1/MIST_-1.5to-2.1.cmd",
    ...
    if pointedsurvey:
        ...
        for n, (_ra, _dec) in enumerate(zip(field_pos[:, 0], field_pos[:, 1])):
            # pointed_data/ prefix will be added by function
            ofile = f"{c['label']}_{n}.hdf5"
            _isfile = fileExists(f"pointed_data/{ofile}")
            if _isfile and clobber is True:
                print("File exists and clobber")
                get_field_wsdb(_ra, _dec, ofile)
            elif _isfile and clobber is False:
        ...
        df.add_virtual_columns_celestial(
            ...
            long_out="l",
            lat_out="b",
            name_prefix="__celestial_eq2gal",
            _matrix="eq2gal",
        )
    ...
    # %%
    np.savetxt(
        f"{sname}.isocused.dat",
        np.array([iiG, iiRP, iig, iii, iiz, np.zeros_like(iiG) + mdmod]).T,
        fmt=["%6.4f"] * 6,
    )
    polG = mkpol(mdmod, c["ddist_G"], iiG, iiRP, offset=c["GRPoff"], p=c["p_G"])
    polgi = mkpol(mdmod, c["ddist_gi"], iig, iii, offset=c["gioff"], p=c["p_gi"])
    poliz = mkpol(
        mdmod, 0.5, iii, iiz, offset=c["gioff"], p=[0.005, 24.6, 1.5, 13.5, 0.9]
    )  # Not used

    df["inG"] = df.geo.inside_polygon(df.dG0 - df.dRP0, df.dG0, polG[:, 0], polG[:, 1])
    df["ingi"] = df.geo.inside_polygon(
        df.dg0 - df.di0, df.dg0, polgi[:, 0], polgi[:, 1]
    )
    df["iniz"] = df.geo.inside_polygon(
        df.di0 - df.dz0, df.di0, poliz[:, 0], poliz[:, 1]
    )
    df["inHB"] = (
        (df.dG0 > c["HBG"][0]) * (df.dG0 < c["HBG"][1]) * (df.dG0 - df.dRP0 < 0.45)
    )

    # Add magnitude cut from Gaia to reach target density
    df["inG"] = (
        df.inG
        * (df.phot_g_mean_mag < c["maglimG"][1])
        * (df.phot_g_mean_mag > c["maglimG"][0])
    )
    ...
    if haspmpoly is not False:
        print("Doing the polygon PM cut")
        df["pmcut"] = df.geo.inside_polygon(
            df.rpmphi1, df.rpmphi2, pmpoly[:, 0], pmpoly[:, 1]
        )
